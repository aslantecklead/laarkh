...
async def test_redis_connection():
    ...
    try:
        ...
        assert await redis_client.ping(), "Не удалось подключиться к Redis"
...
@pytest.mark.asyncio
async def test_rate_limit_functionality():
    ...
    final_count = int(await redis_client.get(rate_limit_key))
...
async def test_concurrent_download_lock():
    ...
    await redis_client.delete(downloading_key)
    await redis_client.delete(status_key)
    
    # Пробуем установить блокировку первый раз
    is_downloading_first = await redis_client.set(downloading_key, "1", ex=expire_time, nx=True)
    ...
    await redis_client.setex(status_key, expire_time, "downloading")
    
    # Пробуем установить блокировку второй раз (должно провалиться)
    is_downloading_second = await redis_client.set(downloading_key, "1", ex=expire_time, nx=True)
    ...
    value = await redis_client.get(downloading_key)
    assert value == "1", f"Значение блокировки изменилось: ожидается '1', получено {value}"
    
    # Проверяем TTL
    ttl = await redis_client.ttl(downloading_key)
