...
class ListAvailableVideosUseCase:
    ...
    async def execute(self, force_refresh: bool = False) -> Tuple[List[Dict[str, Any]], str]:
        cached = await self._cache.get_videos()
        if cached is not None and not force_refresh:
            return cached, "cache"

        try:
            videos = await self._repository.list_available_videos()
        except Exception:
            if cached is not None:
                return cached, "cache_stale"
            raise

        await self._cache.set_videos(videos, self._ttl_sec)
        return videos, "db"
